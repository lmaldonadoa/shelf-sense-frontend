...
import { Check, X, Maximize2, ArrowRight, Search, Filter } from "lucide-react";
    ...
    <div className="space-y-4" onKeyDown={handleKeyPress} tabIndex={0}>
      ...
      {currentItem && (
        <div className="grid grid-cols-4 gap-4 h-[600px]">
          ...
          {/* Col 2: Candidatos top */}
          <Card className="border-white/10 bg-white/5">
            <CardHeader>
              <CardTitle className="text-sm">Top Candidatos</CardTitle>
            </CardHeader>
              ...
              {currentItem.top_candidates.map((cand, idx) => (
                <div
                  ...
                  className={`p-2 rounded border ${
                    idx === 0 ? "border-green-500 bg-green-950/20" : "border-white/10 bg-white/3"
                  }`}
                >
                  <p className="font-mono text-xs font-semibold truncate">{cand.sku_id}</p>
                  <p className="text-xs text-white/60 truncate">{cand.nombre}</p>
                  <div className="flex items-center justify-between mt-1">
                    <Badge variant="outline" className="text-xs">
                      {(cand.score * 100).toFixed(0)}%
                    </Badge>
                    {idx === 0 && <span className="text-xs text-green-300">Top</span>}
                  </div>
                </div>
              ))}
            </CardContent>
